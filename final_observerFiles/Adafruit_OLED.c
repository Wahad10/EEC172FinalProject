...
static unsigned char g_ucTxBuff[1];

//*****************************************************************************
//writes a command by pulling d/c low
void writeCommand(unsigned char c) {

    //dc,cs down,data,cs up
    g_ucTxBuff[0] = c;

    //write d/c command for pin of dc
    GPIOPinWrite(GPIOA2_BASE, 0x40, 0);
    //write cs command pull down
    GPIOPinWrite(GPIOA3_BASE, 0x80, 0);

    SPITransfer(GSPI_BASE,g_ucTxBuff,0,1,
                SPI_CS_ENABLE|SPI_CS_DISABLE);

    //write cs command pull up
    GPIOPinWrite(GPIOA3_BASE, 0x80, 0xFF);

}
//*****************************************************************************
//writes data by pulling d/c high
void writeData(unsigned char c) {

    g_ucTxBuff[0] = c;

    //write d/c command for pin of d/c
    GPIOPinWrite(GPIOA2_BASE, 0x40, 0xFF);
    //write cs command pull down
    GPIOPinWrite(GPIOA3_BASE, 0x80, 0);
